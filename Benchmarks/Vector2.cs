...
namespace Benchmarks
{
    public class Vector2
    {
        ...
        {
            return Mathf.Sqrt(Mathf.Pow(Mathf.Abs(from.x - to.x), 2) + Mathf.Pow(Mathf.Abs(from.y - to.y), 2));
        }

        private static Vector2 Benchtest1(float signedAngle)
        {
            return (signedAngle > 0) ? (Mathf.Abs(signedAngle) < 22.5f) ? Vector2.up : (Mathf.Abs(signedAngle) < 67.5f) ? new Vector2(-.7f, .7f) : (Mathf.Abs(signedAngle) < 112.5f) ? Vector2.left : (Mathf.Abs(signedAngle) < 157.5f) ? new Vector2(-.7f, -.7f) : Vector2.down : (Mathf.Abs(signedAngle) < 22.5f) ? Vector2.up : (Mathf.Abs(signedAngle) < 67.5f) ? new Vector2(.7f, .7f) : (Mathf.Abs(signedAngle) < 112.5f) ? Vector2.right : (Mathf.Abs(signedAngle) < 157.5f) ? new Vector2(.7f, -.7f) : Vector2.down;
        }

        private static Vector2 Benchtest2(float signedAngle)
        {
            Vector2 instPlace;
            if (Mathf.Abs(signedAngle) > 22.5f && Mathf.Abs(signedAngle) < 67.5f)
                instPlace = (signedAngle > 0) ? new Vector2(-.7f, .7f) : new Vector2(.7f, .7f);
            else if (Mathf.Abs(signedAngle) > 67.5f && Mathf.Abs(signedAngle) < 112.5f)
                instPlace = (signedAngle > 0) ? Vector2.left : Vector2.right;
            else if (Mathf.Abs(signedAngle) > 112.5f && Mathf.Abs(signedAngle) < 157.5f)
                instPlace = (signedAngle > 0) ? new Vector2(-.7f, -.7f) : new Vector2(.7f, -.7f);
            else
                instPlace = (Mathf.Abs(signedAngle) > 90) ? Vector2.down : Vector2.up;
            return instPlace;
        }
